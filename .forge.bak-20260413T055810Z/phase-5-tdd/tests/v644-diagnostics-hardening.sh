...
for f in "$SKILL" "$MCP_DETECTION" \
          ...
          "$REPO_ROOT/skills/init/SKILL.md"; do
  if [ ! -f "$f" ]; then
...
# Search onboard, scaffold, init for bare path references that are not inside
# a resolution/path-note/Glob block.
bare_refs=$(grep -rn 'docs/reference/trackers\.md' \
    "$REPO_ROOT/skills/onboard/SKILL.md" \
    "$REPO_ROOT/skills/scaffold/SKILL.md" \
    "$REPO_ROOT/skills/init/SKILL.md" \
...
if [ -z "$bare_refs" ]; then
  echo "OK (AC-2): No bare trackers.md references found in onboard/scaffold/init"
else
...
# -----------------------------------------------------------------------
echo "--- T4 (AC-5): 3-layer Glob resolution in onboard/scaffold/init ---"
...
if grep -qE 'not found.*hardcoded|not found.*default|If not found' \
    "$REPO_ROOT/skills/init/SKILL.md"; then
  echo "OK (AC-4): init — fallback message for missing trackers.md present"
else
  fail "AC-4: skills/init/SKILL.md missing fallback message when trackers.md not found"
fi
