#!/usr/bin/env bash
# v10.1.1: regression test for grep -A window in check_dispatch_witness.
...
# CLAUDE.md (mandates 2-5 items). Pre-v10.1.1 grep -A 8 caught lines 1-8
# from stage key match; dispatch_witness lived at line ~13+ in pretty-
# printed JSON. v10.1.1 raises window to -A 30 (6x safety margin).
#
...
if grep -qE 'grep[[:space:]]+-A[[:space:]]+8[^0-9]' "$LIB"; then
    fail "ASSERT-4: $LIB still contains 'grep -A 8' — v10.1.1 fix regression"
fi
