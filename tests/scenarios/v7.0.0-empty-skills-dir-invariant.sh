...
# the skill count or otherwise break the 18-directory invariant.
...
# Functional check 5: total skill directory count is 18 (no inflation from orphan dirs)
...
if [ "$skill_count" != "18" ]; then
  fail "skills/ contains $skill_count directories, expected exactly 18"
fi

[ "$FAIL" -eq 0 ] && echo "PASS: AC-COUNTS-10 — no empty orphan directories under skills/; skill count is 18"
exit "$FAIL"
