...
FILE="$REPO_ROOT/docs/plans/roadmap.md"

if [ ! -f "$FILE" ]; then
  echo "FAIL: v9-5-roadmap-corrected — docs/plans/roadmap.md not found"
  exit 1
fi
...
if grep -qF '17 → 16' "$REPO_ROOT/docs/plans/roadmap.md"; then
...
if grep -qE '17[[:space:]]*->[[:space:]]*16[[:space:]]*(core|contracts)' "$FILE"; then
  fail "docs/plans/roadmap.md still contains '17 -> 16 core' (incorrect claim)"
else
...
if grep -qE '17 core \(unchanged' "$FILE"; then
  echo "PASS: docs/plans/roadmap.md contains '17 core (unchanged)' corrected claim"
else
  fail "docs/plans/roadmap.md does not contain '17 core (unchanged)' corrected claim"
fi
