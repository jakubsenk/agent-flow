...
if command -v jq > /dev/null 2>&1; then
  ...
  if grep -qF '"outcome": "paused"' "$TMPDIR_TEST/.agent-flow/state.json"; then
...
if command -v jq > /dev/null 2>&1; then
  ...
  if grep -qF '"last_completed_step": "04-fixer-reviewer-loop"' "$TMPDIR_TEST/.agent-flow/state.json"; then
