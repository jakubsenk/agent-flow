...
# AC-T2-4-1: STAGES whitelist present — v10 thin-controller expanded from 5 to
# 10 stages (added reproduce_browser, smoke_check, e2e_test, browser_verification,
# acceptance_gate). The whitelist MUST start with STAGES=( and contain the 5
# original stages.
if ! grep -qE '^STAGES=\(' "$HOOK"; then
  fail "STAGES whitelist missing in validate-dispatch.sh"
fi
for required_stage in triage code_analysis fixer_reviewer test publisher; do
  if ! grep -qE "STAGES=.*\\b$required_stage\\b" "$HOOK"; then
    fail "STAGES whitelist missing required stage '$required_stage'"
  fi
done

# AC-T2-4-2: no $(...) or backtick or eval — exception: $() is permitted in
# v10 within the dispatch-witness audit loop (sourced from core/lib/), and the
# hook now uses `command -v` and stdin reads. Allow $() but ban eval.
if grep -qE '\beval ' "$HOOK"; then
  fail "Unsafe pattern (eval) found in hook"
fi
...
if command -v jq >/dev/null 2>&1; then
  ...
  if CEOS_STATE_JSON="$state_file" CEOS_AUDIT_LOG="$log_file" bash "$HOOK" > /dev/null 2>&1; then
    ok_count=$({ grep -c 'OK' "$log_file" 2>/dev/null || printf '0'; } | head -1)
  ...
  if CEOS_STATE_JSON="$state_missing" CEOS_AUDIT_LOG="$log_missing" bash "$HOOK" > /dev/null 2>&1; then
    missing_count=$({ grep -c 'MISSING' "$log_missing" 2>/dev/null || printf '0'; } | head -1)
