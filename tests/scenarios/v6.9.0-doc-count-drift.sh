...
echo "--- Assertion 1 (AC-064): CLAUDE.md says '17 shared pipeline pattern contracts' ---"
if grep -qF '17 shared pipeline pattern contracts' "$CLAUDE_MD"; then
  echo "OK (AC-064): CLAUDE.md updated to 17 core contracts"
else
  fail "AC-064: CLAUDE.md missing '17 shared pipeline pattern contracts'"
fi
if grep -qF '16 shared pipeline pattern contracts' "$CLAUDE_MD"; then
  fail "AC-064 NEGATIVE: CLAUDE.md still contains stale '16 shared pipeline pattern contracts'"
fi

echo "--- Assertion 2 (AC-064): prompt-injection-protection.sh updated for 17 agents ---"
if [ -f "$PROMPT_INJ" ]; then
  if grep -qE '\-eq 17|expected 17|enumerated 17|COUNT.*17|17.*agent' "$PROMPT_INJ" 2>/dev/null; then
    echo "OK (AC-064): prompt-injection-protection.sh references 17 agents (current agent count)"
  else
    fail "AC-064: prompt-injection-protection.sh does not reference 17 as expected agent count"
  fi
fi

echo "--- Assertion 3 (AC-064a): CLAUDE.md says '18 optional config sections in total' ---"
if grep -qF '18 optional config sections in total' "$CLAUDE_MD"; then
  echo "OK (AC-064a): CLAUDE.md says 18 optional config sections"
else
  fail "AC-064a: CLAUDE.md missing '18 optional config sections in total'"
fi
...
echo "--- Assertion 5 (AC-064a NEGATIVE): no stale 19 optional ---"
if grep -qF '19 optional config sections in total' "$CLAUDE_MD"; then
  fail "AC-064a: CLAUDE.md still has stale '19 optional config sections in total'"
fi
...
[ "$core_count" -eq 17 ] || fail "core/*.md count = $core_count, expected 17 (CLAUDE.md says 17)"
...
[ "$agents_count" -eq 17 ] || fail "agents/*.md count = $agents_count, expected 17 (CLAUDE.md says 17)"
...
[ "$skills_count" -eq 18 ] || fail "skills/ directory count = $skills_count, expected 18 (v9.5.0: estimate/migrate-config/pipeline-status/scaffold-validate removed)"
...
if [ "${optional_count:-0}" -gt 0 ] 2>/dev/null; then
  [ "$optional_count" -eq 18 ] || fail "Optional sections table row count = $optional_count, expected 18"
  ...
  if ! grep -qF '18 optional config sections in total' "$CLAUDE_MD"; then
    fail "CLAUDE.md does not state '18 optional config sections in total'"
  fi
fi

[ "$FAIL" -eq 0 ] && echo "PASS: v6.9.0/v6.10.0 doc count drift checks passed (17 core, 17 agents, 18 skills, 18 optional)"
exit "$FAIL"
