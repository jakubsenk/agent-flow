...
#   5) The library is ≤140 lines per REQ-REL-LIB-BUDGET (v10.1.0; raised from
...
# 5) Line ceiling: ≤140 per REQ-REL-LIB-BUDGET (v10.1.0; raised from REQ-B-1 ceiling of 120 to accommodate POL-2 helpers)
LIB_LINES=$(count_lines "$LIB")
if [ "$LIB_LINES" -gt 140 ]; then
  fail "FC-2.lib-size: $LIB = ${LIB_LINES}L (ceiling 140 per REQ-REL-LIB-BUDGET v10.1.0)"
fi
