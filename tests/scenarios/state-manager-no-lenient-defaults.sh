...
if grep -qF "Missing \`tokens_used\` / \`duration_ms\` / \`tool_uses\` -> treat as \`0\`" "$FILE"; then
  echo "FAIL: v9-5-state-manager-no-lenient-defaults — legacy lenient-defaults fallback still present in core/state-manager.md"
  exit 1
else
  echo "PASS: v9-5-state-manager-no-lenient-defaults — legacy lenient-defaults line absent from core/state-manager.md"
  exit 0
fi
