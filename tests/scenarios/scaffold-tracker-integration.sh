...
# G-14: Display message format "Transitioned ... issues to Done"
if ! grep -qE 'Transitioned.*issues.*to Done' "$SCAFFOLD_SKILL"; then
  fail "G-14: Step 8b missing display message with 'Transitioned ... issues to Done'"
fi
...
# G-17: Explicit close — close ALL story sub-issues for ALL trackers (no cascade assumption)
if ! grep -q 'Close each story sub-issue individually for ALL tracker types\|close.*story.*individually.*ALL' "$SCAFFOLD_SKILL"; then
  fail "G-17: Step 8b missing explicit close behavior ('close each story sub-issue individually for ALL tracker types')"
fi
