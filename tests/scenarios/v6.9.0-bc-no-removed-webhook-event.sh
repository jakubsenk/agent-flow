...
# CLAUDE.md §Webhook Payloads: "pr-created" and "ceos-agents-block" are never renamed or removed.
# "issue-blocked" is the On events config alias; "ceos-agents-block" is the payload event name.
REQUIRED_EVENTS=(
  "pr-created"
  "ceos-agents-block"
  "pipeline-started"
...
# Mutation guard: CLAUDE.md explicitly contracts pr-created + ceos-agents-block as never-removed
if ! grep -qF 'pr-created' "$REPO_ROOT/CLAUDE.md"; then
  fail "CLAUDE.md missing 'pr-created' webhook event"
fi
if ! grep -qF 'ceos-agents-block' "$REPO_ROOT/CLAUDE.md"; then
  fail "CLAUDE.md missing 'ceos-agents-block' webhook event (backward-compat contract)"
fi
