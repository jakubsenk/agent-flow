...
# Iterates agents dynamically asserting Constraints section present.
set -uo pipefail
...
# Enumerate all agents dynamically (excluding README.md)
...
# Mutation guard: count must equal filesystem source of truth
agent_count=$(find "$REPO_ROOT/agents" -maxdepth 1 -name '*.md' -not -name 'README.md' -type f | wc -l | tr -d ' ')
EXPECTED=$(find "$REPO_ROOT/agents" -maxdepth 1 -name '*.md' -not -name 'README.md' -type f | wc -l | tr -d ' ')
[ "$agent_count" -eq "$EXPECTED" ] || fail "Expected $EXPECTED agents (filesystem count), found $agent_count"

[ "$FAIL" -eq 0 ] && echo "PASS: all $agent_count agents have required output contract sections"
exit "$FAIL"
