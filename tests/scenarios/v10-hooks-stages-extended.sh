...
#   B) STAGES contains ALL 10 v10.0.0 stages (sorted-set equality):
...
# D. Sanity sub-asserts for the 5 NEW stages introduced in v10
for new_stage in reproduce_browser smoke_check e2e_test browser_verification acceptance_gate; do
  if ! printf '%s\n' $NAMES | grep -qw "$new_stage"; then
    fail "FC-4.D: STAGES missing NEW v10 stage '${new_stage}'"
  fi
done
...
if [ "$FAIL" -eq 0 ]; then
  echo "PASS: v10-hooks-stages-extended — STAGES has 10 v10.0.0 stages; sources stage-invariant.sh"
  exit 0
fi
exit 1
