#!/bin/bash
# PURPOSE: Replacement for section-order.sh using the v9.0.0 17-agent roster and tolerating the
...
# EXPECTED ON v8.0.0: FAIL (stack-selector.md present = agent count mismatch)
# EXPECTED ON v9.0.0: PASS (17 agents with correct section order including optional Output Contract)
set -uo pipefail
...
# Post-v9.0.0 roster: 17 agents (stack-selector deleted)
AGENTS=(
...
for agent in "${AGENTS[@]}"; do
  ...
  # last ## Process line and ## Constraints (mirrors v9-output-contract-position.sh logic)
...
[ "$FAIL" -eq 0 ] && echo "PASS: AC-H-080, AC-H-081 — all 17 v9.0.0 agents follow Goal -> Expertise -> Process -> [Output Contract] -> Constraints section order"
exit "$FAIL"
