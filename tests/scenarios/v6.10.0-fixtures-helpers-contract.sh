...
count=$(grep -cE '^(make_state_json|make_state_json_bash|setup_scratch|require_jq)\(\)' "$FIXTURES" 2>/dev/null || echo 0)
[ "$count" -eq 4 ] || fail "Expected 4 function declarations in fixtures.sh, got $count"

# AC-T1-4-2: sourcing declares exactly the 4 functions
declared=$(bash -c ". '$FIXTURES'; declare -F" 2>/dev/null | grep -cE 'make_state_json[^_]|make_state_json$|make_state_json_bash|setup_scratch|require_jq' || echo 0)
[ "$declared" -eq 4 ] || fail "Expected 4 functions declared after sourcing, got $declared"
...
[ "$FAIL" -eq 0 ] && echo "PASS: fixtures.sh API contract verified (4 helpers)"
exit "$FAIL"
