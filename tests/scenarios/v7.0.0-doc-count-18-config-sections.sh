...
# Functional check 2: README.md has "18 optional config sections", not 19
if ! grep -qF '18 optional config sections' README.md 2>/dev/null; then
  fail "README.md: '18 optional config sections' not found"
fi
if grep -qE '\b19 optional (config )?sections\b' README.md 2>/dev/null; then
...
# Functional check 4: agent count is 17 (v9.0.0 deleted stack-selector and merged 3 pairs)
agent_count=$(find agents -maxdepth 1 -mindepth 1 -name '*.md' | wc -l | tr -d ' ')
if [ "$agent_count" != "17" ]; then
  fail "Filesystem: found $agent_count agent .md files under agents/, expected 17"
fi
...
[ "$FAIL" -eq 0 ] && echo "PASS: AC-COUNTS-2,3,5,9 — all anchor files show 18 optional config sections; agent count is 17"
exit "$FAIL"
