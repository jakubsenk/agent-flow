#!/usr/bin/env bash
# Test: Agent model assignments match the CLAUDE.md table (17 agents)
...
# sonnet agents: analyst, test-engineer, spec-analyst, scaffolder, acceptance-gate,
#                browser-agent, deployment-verifier, backlog-creator, sprint-planner
SONNET_AGENTS=(
  analyst test-engineer spec-analyst scaffolder acceptance-gate
  browser-agent deployment-verifier backlog-creator sprint-planner
)
...
[ "$FAIL" -eq 0 ] && echo "PASS: All 17 agents have correct model assignments (opus/sonnet/haiku)"
exit "$FAIL"
