...
# Expected RED phase (on v10.0.0): PASS — all agents and custom-agent examples
#   already contain ## Step Completion Invariants (v10.0.0 landed them).
#   Scenario RED phase triggers when a future file is added without the section.
...
for i in "${!DIRS[@]}"; do
  ...
  # [ASSERT-2] Directory must exist (FAIL not SKIP — ships in-repo since v10.0.0)
  if [ ! -d "$d" ]; then
    fail "FC-REL-2.dir: $d missing — expected in-repo since v10.0.0"
    continue
  fi
