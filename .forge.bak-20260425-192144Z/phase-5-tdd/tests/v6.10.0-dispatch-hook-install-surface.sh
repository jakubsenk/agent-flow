...
# skills/init/SKILL.md has NO auto-install mention,
...
INIT="$REPO_ROOT/skills/init/SKILL.md"
if [ -f "$INIT" ]; then
  if grep -qiE 'PostToolUse|validate-dispatch' "$INIT"; then
    fail "skills/init/SKILL.md must NOT auto-install PostToolUse hook (forbidden per REQ-T2-6)"
  fi
fi
