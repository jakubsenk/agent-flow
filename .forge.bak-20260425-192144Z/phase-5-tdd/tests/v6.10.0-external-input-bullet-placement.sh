...
for agent in "${TARGET_AGENTS[@]}"; do
  ...
  # AC-T3-4-1: bullet must be in ## Constraints section
  in_constraints=0
  found_bullet=0
  while IFS= read -r line; do
    if echo "$line" | grep -qE '^## Constraints'; then
      in_constraints=1
    elif echo "$line" | grep -qE '^## [A-Z]'; then
      in_constraints=0
    fi
    if [ "$in_constraints" -eq 1 ] && echo "$line" | grep -qF 'EXTERNAL INPUT START'; then
      found_bullet=1
    fi
  done < "$f"
  ...
  if [ "$agent" = "sprint-planner" ] || [ "$agent" = "publisher" ]; then
    fence_line=$(grep -n '^\`\`\`' "$f" | tail -1 | cut -d: -f1)
