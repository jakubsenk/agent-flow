#!/usr/bin/env bash
# Test: CLAUDE.md claims 21 agents, skills/ has 28 SKILL.md files,
...
# -----------------------------------------------------------------------
# 2. CLAUDE.md claims 21 agents
# -----------------------------------------------------------------------
agents_claimed=$(grep '`agents/`' "$CLAUDE_MD" | grep -oE '[0-9]+' | head -1)
if [ -z "$agents_claimed" ]; then
  fail "Could not find agent count claim in CLAUDE.md (expected '21 agent definitions')"
elif [ "$agents_claimed" -ne 21 ]; then
  fail "CLAUDE.md claims $agents_claimed agents but expected 21 (19 existing + backlog-creator + sprint-planner)"
fi

# -----------------------------------------------------------------------
# 3. skills/ directory has 28 SKILL.md files
# -----------------------------------------------------------------------
...
if [ "$skills_fs" -ne 28 ]; then
  fail "skills/ has $skills_fs SKILL.md files but expected 28 (26 existing + create-backlog + sprint-plan)"
fi

if [ -n "$skills_claimed" ] && [ "$skills_claimed" -ne 28 ]; then
  fail "CLAUDE.md claims $skills_claimed skills but expected 28"
fi
...
[ "$FAIL" -eq 0 ] && echo "PASS: CLAUDE.md claims 21 agents, skills/ has 28 SKILL.md files, model table includes backlog-creator and sprint-planner under sonnet"
exit "$FAIL"
