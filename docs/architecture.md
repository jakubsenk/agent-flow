...
graph TD
    ...
    subgraph "ceos-agents Plugin"
        ...
        subgraph "Customization Layer (v8.0.0)"
...
| `--step-mode` | **New in v8.0.0** | Pause after each agent step (analogous to debugger step-through); operator prompted "Continue / Skip / Abort" at every `steps/*.md` boundary |
...
The scaffold pipeline (scaffold: 8 steps) creates a new project from scratch. In v2 mode (default), it generates a specification, builds the skeleton, and implements all features:
...
- Scaffolder reads tech stack from spec/README.md (v2 mode) or from skill-supplied stack flags (--no-implement)
...
- With `--no-implement`: infrastructure declaration → scaffolder (with stack flags) → validate → git init + push (v3.x behavior)
...
### Webhook Reliability and Circuit Breaker (v6.9.0+)
...
The state schema (`state/schema.md`) uses `schema_version: "1.0"` throughout v8.0.0. All v8.0.0 additions are **additive** — backward-compatible reads from v7.x pipelines continue without modification.

v8.0.0 additive keys in `state.json`:
