...
- If `.mcp.json` exists in CWD:
  - If `--update` → parse existing config, preserve non-agent-flow servers
...
2. If file does NOT exist → display: `"File not found at {path}. Please check the path and try again."` → re-ask for the path. Max 3 attempts, then STOP with error: `"gitea-mcp binary not found. Download it from https://gitea.com/gitea/gitea-mcp/releases and re-run /agent-flow:setup-mcp."`
...
1. **Pre-flight: binary/command existence check** (for stdio servers — `gitea` binary, `redmine` uvx, YouTrack fallback npx):
   ...
   - If command not found → `"[FAIL] {server_name}: command '{command}' not found. Re-run /agent-flow:setup-mcp to fix the setup."` Skip connectivity check for this server.
