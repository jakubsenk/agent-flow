...
# Must document tolerating missing/absent usage fields
if ! grep -qiE 'tolerat|backward.?compat|absent.*field|missing.*field|v6\.7|legacy' "$FILE"; then
...
[ "$FAIL" -eq 0 ] && echo "PASS: AC-20 — resume-ticket/SKILL.md tolerates v6.7.x state.json"
exit "$FAIL"
