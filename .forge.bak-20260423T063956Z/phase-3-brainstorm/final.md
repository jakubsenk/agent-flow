...
## Gate 1 — User Decisions (2026-04-20)

| Q | User choice | Effect on spec |
|---|------------|----------------|
| Q1 (public mirror) | (b) `.invalid` placeholder + roadmap entry for canonical URL | matches Judge default — A3 partial |
| Q2 (SECURITY.md secondary contact) | (c) accept SPOF, secondary in v6.9.1 + roadmap entry | matches Judge default — A2 ships with primary contact only |
| Q3 (core contract count 15→16) | (a) ADOPT `core/agent-states.md`, accept doc drift fix in 8 places | matches Judge default — count change goes through |
| Q4 (`core/snippets/` namespace) | **(b) ADOPT ALL 5 snippets** | **DEVIATION** — Judge default was (c) PARTIAL (webhook-curl only). User: "udelal bych hned vse at to mame ciste". Phase 4 spec MUST include all 5 snippet files: `core/snippets/webhook-curl.md` (20 sites), `core/snippets/issue-id-validation.md` (4 sites), `core/snippets/metrics-json-schema.md` (1 site), `core/snippets/pipeline-completion.md` (3 sites), `core/snippets/architecture-freshness.md` (2 sites). Top-level core-contracts count stays 15→16 (snippets are sub-namespace, do not count toward `ls core/*.md`). |

**Roadmap entries required (for Phase 7 to write):**
- v6.9.1 entry: "Replace `https://example.invalid/ceos-agents.git` placeholder in `plugin.json.repository` with canonical public mirror URL once provisioned (gated on: mirror exists + DNS resolves + HTTP 200 + org name confirmed)"
- v6.9.1 entry: "SECURITY.md secondary contact channel — add personal/forwarder email or migrate to GitHub Security Advisories Private Vulnerability Reporting once mirror is provisioned"

**Gate 1 status:** APPROVED. Pipeline status → running. Advance to Phase 4 (Spec).
