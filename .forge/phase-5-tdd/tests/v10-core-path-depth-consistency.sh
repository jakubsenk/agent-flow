...
if [ -n "$TMPDIR_CF" ]; then
  # Copy only skills/ and agents/ to keep it lightweight
  cp -r "$REPO_ROOT/skills" "$TMPDIR_CF/" 2>/dev/null || true
  cp -r "$REPO_ROOT/agents" "$TMPDIR_CF/" 2>/dev/null || true

  # Copy this test script into a plausible location the counterfactual can call
  mkdir -p "$TMPDIR_CF/tests/scenarios"
  cp "$0" "$TMPDIR_CF/tests/scenarios/v10-core-path-depth-consistency.sh" 2>/dev/null || true

  # Inject a depth violation: change one ../../../core/ to ../../core/ in a steps file
  # Pick a stable target that is likely to exist after Phase 7 rewrite
  target_file=""
  for candidate in \
      "$TMPDIR_CF/skills/fix-bugs/steps/01-triage.md" \
      "$TMPDIR_CF/skills/implement-feature/steps/01-spec-analysis.md" \
      "$TMPDIR_CF/skills/analyze-bug/steps/01-analyze.md"; do
    if [ -f "$candidate" ]; then
  ...
  if [ -z "$target_file" ]; then
    # No suitable steps file found — Phase 7 hasn't landed yet, fixture is pre-rewrite
    # This is acceptable for TDD: mark as advisory
    echo "[INFO] ASSERT-6: no rewritten steps file found in fixture (Phase 7 not yet applied)"
    echo "[PASS] ASSERT-6: counterfactual skipped — pre-Phase-7 tree has no depth-3 refs to corrupt"
    PASS_COUNT=$((PASS_COUNT + 1))
  else
    # Corrupt: replace first ../../../core/ with ../../core/ in that file
    # Use portable sed (no GNU -i without backup)
    sed 's|\.\./\.\./\.\./core/|../../core/|' "$target_file" > "$target_file.patched" && \
      mv "$target_file.patched" "$target_file"

    # Run this script against the corrupted tree — it must exit non-zero
    cf_rc=0
    CEOS_REPO_ROOT="$TMPDIR_CF" bash "$TMPDIR_CF/tests/scenarios/v10-core-path-depth-consistency.sh" \
      >/dev/null 2>&1 || cf_rc=$?

    if [ "$cf_rc" -ne 0 ]; then
      echo "[PASS] ASSERT-6: counterfactual correctly caught depth violation (exit $cf_rc)"
      PASS_COUNT=$((PASS_COUNT + 1))
    else
      echo "[FAIL] ASSERT-6: counterfactual passed when it should have failed — lint is blind to violations"
      FAIL_COUNT=$((FAIL_COUNT + 1))
    fi
  fi
fi
