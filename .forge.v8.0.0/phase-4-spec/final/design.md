...
## Section 4.2 (v8.0.0 supplement) — Step override resolution and /setup-agents spec

### Step override resolution semantics

When a project-local file `customization/steps/{skill}/{NN-name}.md` exists, it is loaded
**instead of** the plugin-default `skills/{skill}/steps/{NN-name}.md`. The override is
**replace-only**: the override body replaces the default step body — no merge, no insert,
no partial patch. The plugin does NOT append the default step body to the override; only the
override file content is dispatched to the agent.

At dispatch time, the skill emits exactly one `[INFO]` log line:

```
[INFO] Step override active: customization/steps/{skill}/{NN-name}.md (replaces plugin default)
```

This log is emitted **only when an override file is present and loaded**. When no override
exists and the plugin default step is used, no `[INFO] Step override active:` log line is
emitted. The absence of the log line confirms the default-step (no-override) path. Logging is
performed at dispatch time, before the step content is passed to the agent.

### /setup-agents spec — generated TOML header

Every TOML file written by `/setup-agents` MUST begin with a header line matching:
...
This `# generated:` header serves as a machine-readable marker distinguishing auto-generated
files from user-edited overlays. When this header is present, `/setup-agents --refresh`
may overwrite the file without prompting. When absent, the file is treated as user-edited
and the skill will prompt for confirmation before overwriting.

### /setup-agents spec — preview diff before write

In default mode (without `--yolo`), `/setup-agents` MUST display a preview diff of each
TOML file before writing it. The flow is:

1. Generate proposed TOML content in memory
2. If the target file already exists, display a unified diff (proposed vs existing)
3. If the target file does not exist, display the full proposed content as a creation preview
4. Prompt: `Write file? [y/n/s(skip)]` — UNLESS `--yolo` flag is active
5. If `--yolo`, write all files without prompting (no preview displayed)

The purpose of the preview diff step is to allow the operator to review and approve each
generated overlay before it is committed to disk.

---

## Section 10 (v8.0.0 supplement) — Mode framework, overlay precedence, and step-mode §5.2

### Overlay precedence rule

The TOML overlay **always wins** (overlay wins over plugin default) on any key present in the
overlay file. There is no scenario in which the plugin default takes priority over an explicit
overlay key. The rule is simple: if the overlay defines a key, that value is used; if it does
not define a key, the plugin default is inherited. This "overlay wins" contract applies to all
three tiers — scalar override (Tier 1), array append (Tier 2 is additive not a win/lose conflict),
and table deep merge (Tier 3 per-key: overlay key wins, absent keys fall back to plugin default).

### Overlay deep merge worked example

The `[limits]` table uses key-by-key deep merge. Example:

Plugin default `[limits]` for `reviewer`: `{max_review_iterations=5, max_diff_lines=100}`
...
Result: `{max_review_iterations=3, max_diff_lines=100}` — the overlay value `3` wins for
`max_review_iterations`; `max_diff_lines=100` is inherited from the plugin default (absent key
in overlay inherits from plugin default). This is the canonical deep merge example for Tier 3.

### Step-mode §5.2 — Prompt template and behavioral table

In `--step-mode`, after each step completes, the skill emits the following exact prompt:
...
**Behavioral table for `[c/s/a]` input:**

| Input | Action |
|-------|--------|
| `c` | Continue to next step |
| `s` | Skip remaining gates — switch to yolo for remaining steps; emit `[INFO] step-mode escape: switched to yolo for remaining steps` |
| `a` | Abort — write `state.json` with `outcome=paused, pause_reason=step_mode_abort`; exit 0 |
| empty input | Re-prompt with no default action (prevents accidental Enter-skip) |
| other | Re-prompt with "Invalid input; expected c, s, or a" |

The `s` option (Skip remaining gates) switches `MODE` to `yolo` for all subsequent steps — no
further per-step prompts are shown after the escape. The exact log line is:
