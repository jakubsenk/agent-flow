...
## REQ-MODE (v8.0.0 supplement)

### AC-MODE-005 — Step-mode 's' escape: switched to yolo for remaining steps

WHEN the user inputs `s` at the `[step-mode]` prompt,
THEN `MODE` SHALL be switched to `yolo` for all remaining steps (no further per-step prompts),
AND the log SHALL contain the exact line:
`[INFO] step-mode escape: switched to yolo for remaining steps`.

```bash
# Verify design.md §5.2 (Section 10) documents step-mode escape + switched to yolo
grep -qiE 'step-mode escape.*switched.*yolo|switched.*yolo.*remaining' \
  .forge/phase-4-spec/final/design.md
```

This AC verifies the behavioral contract for the `s` (Skip remaining gates) option in
`--step-mode`. After the escape, subsequent steps execute without per-step prompts — the
effective mode becomes `yolo` for the remainder of the pipeline run.
