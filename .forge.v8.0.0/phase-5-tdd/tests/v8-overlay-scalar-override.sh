...
# ---------------------------------------------------------------------------
# Assertion 4: design.md documents the override precedence rule
# ---------------------------------------------------------------------------
echo "--- Assertion 4: design.md documents overlay wins over plugin default ---"
DESIGN="$REPO_ROOT/.forge/phase-4-spec/final/design.md"
if [ -f "$DESIGN" ]; then
  if grep -qiE 'overlay.*wins|overlay always wins' "$DESIGN"; then
    echo "OK: design.md documents overlay wins precedence"
  else
    fail "design.md does not document overlay-wins-over-plugin-default rule"
  fi
fi
