...
# Canonical 29 skill names (AC-DOC-006)
CANONICAL_SKILLS=(
  analyze-bug autopilot changelog check-deploy check-setup create-backlog
...
# ---------------------------------------------------------------------------
# Assertion 1: all 29 skills present
# ---------------------------------------------------------------------------
echo "--- Assertion 1: all 29 canonical skills in skills.md ---"
...
# ---------------------------------------------------------------------------
echo "--- Assertion 2: setup-agents listed in skills.md (new v8 skill) ---"
if grep -qF 'setup-agents' "$SKILLS_DOC"; then
  echo "OK: setup-agents in skills.md"
else
  fail "skills.md missing setup-agents (new v8 skill)"
fi

# ---------------------------------------------------------------------------
# Assertion 3: no create-pr (removed in v7)
# ---------------------------------------------------------------------------
echo "--- Assertion 3: create-pr NOT in skills.md (removed in v7) ---"
if grep -qF 'create-pr' "$SKILLS_DOC"; then
  fail "skills.md still contains create-pr (should be removed since v7.0.0)"
else
...
# ---------------------------------------------------------------------------
if [ "$FAIL" -eq 0 ]; then
  echo "PASS: AC-DOC-006 — docs/reference/skills.md has exactly 29 canonical skills"
fi
exit "$FAIL"
