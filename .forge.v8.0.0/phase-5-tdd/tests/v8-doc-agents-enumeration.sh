...
# Description: docs/reference/agents.md contains exactly the 18 canonical agent names
...
grep -oE "^\|[[:space:]]*(analyst|fixer|reviewer|acceptance-gate|test-engineer|publisher|rollback-agent|spec-analyst|architect|stack-selector|scaffolder|priority-engine|spec-writer|spec-reviewer|browser-agent|deployment-verifier|backlog-creator|sprint-planner)[[:space:]]*\|" "$AGENTS_DOC" | \
  sed 's/|//g' | tr -d '[:space:]' | sort > "$TMPDIR_TEST/actual.txt"

# ---------------------------------------------------------------------------
# Assertion 1: all expected agents found
# ---------------------------------------------------------------------------
echo "--- Assertion 1: all 18 canonical agents in agents.md table ---"
...
# ---------------------------------------------------------------------------
# Assertion 2: no extra agents beyond 18
# ---------------------------------------------------------------------------
echo "--- Assertion 2: no extra agents beyond canonical 18 ---"
if diff -q "$TMPDIR_TEST/expected.txt" "$TMPDIR_TEST/actual.txt" > /dev/null 2>&1; then
  echo "OK: agents.md table matches exactly canonical 18 agents"
else
  echo "Diff (expected vs actual):"
  diff "$TMPDIR_TEST/expected.txt" "$TMPDIR_TEST/actual.txt" || true
  fail "agents.md table enumeration does not match canonical 18"
fi
...
# ---------------------------------------------------------------------------
if [ "$FAIL" -eq 0 ]; then
  echo "PASS: AC-DOC-005 — docs/reference/agents.md has exactly 18 canonical agent rows"
fi
exit "$FAIL"
