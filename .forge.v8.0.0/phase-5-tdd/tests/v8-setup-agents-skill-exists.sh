...
# ---------------------------------------------------------------------------
# Assertion 4: skill count includes setup-agents (29 total)
# ---------------------------------------------------------------------------
echo "--- Assertion 4: skills/ count is 29 (includes setup-agents) ---"
SKILL_COUNT=$(find "$REPO_ROOT/skills" -maxdepth 2 -name 'SKILL.md' -not -path '*/steps/*' -type f | wc -l)
if [ "$SKILL_COUNT" -eq 29 ]; then
  echo "OK: skills/ contains 29 SKILL.md files"
else
  echo "INFO: skills/ contains $SKILL_COUNT SKILL.md files (expected 29 post-v8.0.0)"
  if [ "$SKILL_COUNT" -lt 28 ]; then
    fail "skills/ has $SKILL_COUNT SKILL.md files — expected 29 post-v8.0.0"
  fi
fi
...
# ---------------------------------------------------------------------------
if [ "$FAIL" -eq 0 ]; then
  echo "PASS: AC-SETUP-001 — skills/setup-agents/SKILL.md exists with correct frontmatter"
fi
exit "$FAIL"
