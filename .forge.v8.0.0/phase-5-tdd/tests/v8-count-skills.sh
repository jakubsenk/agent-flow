...
# ---------------------------------------------------------------------------
echo "--- Assertion: skills/ has exactly 29 SKILL.md files (excluding steps/) ---"
...
if [ "$ACTUAL_COUNT" -eq "$EXPECTED_SKILL_COUNT" ]; then
  ...
  fail "skills/ has $ACTUAL_COUNT SKILL.md files — expected $EXPECTED_SKILL_COUNT post-v8.0.0"
fi

# ---------------------------------------------------------------------------
# Assertion: setup-agents is in the count
# ---------------------------------------------------------------------------
echo "--- Assertion: skills/setup-agents/SKILL.md present (new v8 skill) ---"
if [ -f "$REPO_ROOT/skills/setup-agents/SKILL.md" ]; then
  echo "OK: skills/setup-agents/SKILL.md exists"
else
  fail "skills/setup-agents/SKILL.md missing — required for 29-skill count"
fi
...
# ---------------------------------------------------------------------------
if [ "$FAIL" -eq 0 ]; then
  echo "PASS: AC-CT-002 — skills/ has exactly 29 SKILL.md files"
fi
exit "$FAIL"
