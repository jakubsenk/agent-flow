...
# ---------------------------------------------------------------------------
# Assertion 4: design.md step 4 documents preview flow
# ---------------------------------------------------------------------------
echo "--- Assertion 4: design.md setup-agents flow includes preview diff step ---"
DESIGN="$REPO_ROOT/.forge/phase-4-spec/final/design.md"
if [ -f "$DESIGN" ]; then
  if grep -qiE 'preview.*diff|display.*preview|UNLESS.*\-\-yolo' "$DESIGN"; then
    echo "OK: design.md setup-agents flow includes preview diff"
  else
    fail "design.md setup-agents flow missing preview diff step"
  fi
fi
