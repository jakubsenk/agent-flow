...
# generated: 2026-04-27T10:01:00Z by /setup-agents v8.0.0
[[process_additions]]
step = "after_default"
...
# ---------------------------------------------------------------------------
# Assertion 4: design.md contains the header format
# ---------------------------------------------------------------------------
echo "--- Assertion 4: design.md examples show # generated: header ---"
DESIGN="$REPO_ROOT/.forge/phase-4-spec/final/design.md"
if [ -f "$DESIGN" ]; then
  if grep -qF '# generated:' "$DESIGN"; then
    echo "OK: design.md examples include # generated: header"
  else
    fail "design.md missing # generated: header in examples"
  fi
fi
