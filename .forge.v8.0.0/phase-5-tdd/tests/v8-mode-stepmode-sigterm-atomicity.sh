...
if grep -qiE 'write.*after.*complete|after.*success.*write|last_completed_step.*written' "$FIXBUGS_SKILL"; then
...
if command -v jq > /dev/null 2>&1; then
  LAST=$(jq -r '.last_completed_step' "$TMPDIR_TEST/.ceos-agents/state.json")
