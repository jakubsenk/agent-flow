...
if [ ! -f "$RESUME" ]; then
  fail "skills/resume-ticket/SKILL.md does not exist"
else
  # Must reference plugin_version field
  if ! grep -q "plugin_version" "$RESUME"; then
    fail "skills/resume-ticket/SKILL.md does not reference 'plugin_version'"
  fi

  # Must contain the exact WARN text for major version mismatch
  if ! grep -q "major version mismatch" "$RESUME"; then
    fail "skills/resume-ticket/SKILL.md does not contain 'major version mismatch' warning text"
  fi

  # Must reference plugin.json for reading the current installed version
  if ! grep -q "plugin.json" "$RESUME"; then
    fail "skills/resume-ticket/SKILL.md does not reference 'plugin.json' (current version source)"
  fi

  # The mismatch must be advisory (WARN), not a blocking action
  if ! grep "major version mismatch" "$RESUME" | grep -qi "warn"; then
    fail "skills/resume-ticket/SKILL.md: 'major version mismatch' message does not use WARN (must be advisory, not a block)"
  fi
fi
...
if [ -f "$RESUME" ]; then
  # The skill must explicitly handle the absent/null case
  if ! grep -qiE "absent|null" "$RESUME"; then
    fail "skills/resume-ticket/SKILL.md does not handle absent/null plugin_version case (backwards compatibility guard missing)"
  fi

  # The handling must be silent: skip, not warn
  if ! grep -iE "plugin_version.*(absent|null)|(absent|null).*plugin_version|field is absent|field.*missing|field.*not present" "$RESUME" | grep -qiE "skip|silent|no WARN|no warn"; then
    fail "skills/resume-ticket/SKILL.md: absent/null plugin_version case does not describe silent skip (expected: skip silently / no WARN)"
  fi
fi
