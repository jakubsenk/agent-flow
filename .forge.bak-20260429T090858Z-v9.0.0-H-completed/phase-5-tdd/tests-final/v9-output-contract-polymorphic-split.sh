...
# EXPECTED ON v8.0.0: SKIP (no agent has ## Output Contract yet)
# EXPECTED ON v9.0.0: PASS (all 4 polymorphic agents have correct per-phase sub-blocks)
set -uo pipefail
...
check_polymorphic() {
  ...
  if ! grep -qE '^## Output Contract$' "$agent_file"; then
    echo "SKIP: $agent_name lacks ## Output Contract (v8.0.0 baseline)" >&2
    return 77
  fi
...
# If all 4 agents were skipped, this is the v8.0.0 baseline — exit 77
all_skip=$(( analyst_skip + te_skip + ba_skip + sr_skip ))
if [ "$all_skip" -eq 308 ]; then  # 77 * 4
  echo "SKIP: all 4 polymorphic agents lack ## Output Contract (v8.0.0 baseline — expected)" >&2
  exit 77
fi
