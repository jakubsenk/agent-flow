...
# EXPECTED ON v8.0.0: SKIP (no agent has ## Output Contract yet — every file triggers SKIP-guard)
# EXPECTED ON v9.0.0: PASS (all 17 agents have well-formed Output Contract)
set -uo pipefail
...
# If every agent was skipped, that is expected on v8.0.0 baseline — exit 77
if [ "$checked" -eq 0 ] && [ "$skipped" -gt 0 ]; then
  echo "SKIP: all $skipped agents lack ## Output Contract section (v8.0.0 baseline — expected)" >&2
  exit 77
fi
