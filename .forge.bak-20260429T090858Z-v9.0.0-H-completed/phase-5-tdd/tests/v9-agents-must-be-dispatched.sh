...
#          agents/*.md MUST appear as a subagent_type='ceos-agents:{name}' literal in at
...
# EXPECTED ON v8.0.0: FAIL (stack-selector.md exists but has zero dispatch references)
# EXPECTED ON v9.0.0: PASS (stack-selector deleted; all 17 remaining agents are dispatched)
set -uo pipefail
...
for agent_file in "$AGENTS_DIR"/*.md; do
  ...
  # Build the strict dispatch pattern: subagent_type='ceos-agents:{name}'
  dispatch_pattern="subagent_type='ceos-agents:${frontmatter_name}'"
  ...
  if [ "$found" -eq 0 ]; then
    ...
    # Also catches the v8.0.0 stack-selector orphan on the current codebase
  fi
done
...
# On v8.0.0 this will fail (file exists), which is correct RED behavior
